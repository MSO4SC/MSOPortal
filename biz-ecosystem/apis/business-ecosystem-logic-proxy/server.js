...
app.get('/auth/fiware/callback', passport.authenticate('fiware', {
    ...
    res.redirect('http://192.168.56.23:8000/marketplaceLoggedIn/');
});
...
var checkMongoUp = function (req, res, next) {
    ...
    if (mongoose.connection.readyState !== 1) {
        ...
        res.status(500);
        res.json({
            error: 'It was impossible to connect with the database. Please, try again in a few seconds.'
        });
        res.end();
...
app.use(function (err, req, res, next) {
    ...
    switch (req.accepts([applicationJSON, textHtml])) {
        ...
        case applicationJSON:
            ...
            res.json({
                error: 'Unexpected error. The error has been notified to the administrators.'
            });
            res.end();
